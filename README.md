...
- **大数字/渐变遮罩适配**：大量彩色字形（colored ≥350）的格子强判为"已翻开数字"，避免顶部 UI 渐变/压花底纹把大号数字误判成未翻开或旗帜；低色噪点不再误兜底为旗帜
- **OCR 规则后验校验（trans-app-ocr.md P2）**：识别输出前做扫雷约束校验/修复——任一数字 d 必须满足 旗数≤d≤旗数+未知数；多余旗帜降级为未知（优先保留对邻域破坏最小的旗），不可能数字修正（无旗无候选的孤立数字还原为未知）。因此所有输入截图输出的棋盘均为约束合法，不会出现"大量不合法旗帜/数字"
